import React, { useState } from "react";
import { Image } from "@chakra-ui/react";

const NoImagePlaceHolder = () => {
  return (
    <div className="flex flex-col items-center justify-center pt-5 pb-6">
      <svg
        className="w-8 h-8 mb-4 text-gray-500 dark:text-gray-400"
        aria-hidden="true"
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
        viewBox="0 0 20 16"
      >
        <path
          stroke="currentColor"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth="2"
          d="M13 13h3a3 3 0 0 0 0-6h-.025A5.56 5.56 0 0 0 16 6.5 5.5 5.5 0 0 0 5.207 5.021C5.137 5.017 5.071 5 5 5a4 4 0 0 0 0 8h2.167M10 15V6m0 0L8 8m2-2 2 2"
        />
      </svg>
      <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
        <span className="font-semibold">Click to upload</span>
      </p>
      <p className="text-xs text-gray-500 dark:text-gray-400">
        SVG, PNG, JPG or GIF (MAX. 800x400px)
      </p>
    </div>
  );
};

function FileUploader({ setState }) {
  const [prevImage, setPrevImage] = useState(null);

  // JavaScript function to handle errors
  function handleImage(event) {
    const selectedFile = event.target.files;

    if (selectedFile) {
      // Check if the selected file is an image
      if (!selectedFile[0].type.startsWith("image/")) {
        alert("Please select an image file.");
        // Clear the file input
        event.target.value = null;
        setPrevImage(null);
        setState(null);
        return;
      }

      // Handle the selected image file
      // ...
      setPrevImage(URL.createObjectURL(selectedFile[0]));
      setState(selectedFile);
    }
  }

  return (
    <div className="flex items-center justify-center w-full">
      <label
        htmlFor="dropzone-file" // Change 'for' to 'htmlFor'
        className="flex flex-col items-center justify-center w-full h-40 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
      >
        {!prevImage ? (
          <NoImagePlaceHolder />
        ) : (
          <Image
            alt="image"
            src={prevImage}
            className="w-full h-full object-cover"
          />
        )}
        <input
          onChange={handleImage}
          id="dropzone-file"
          type="file"
          accept="image/*"
          className="hidden"
        />
      </label>
    </div>
  );
}

export default FileUploader;
